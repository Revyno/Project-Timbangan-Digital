...
class FormulasiIotController extends Controller
{
    /**
     * @OA\Get(
     *     path="/api/v1/formulasi/settings",
     *     tags={"Formulasi Pasuruan"},
     *     summary="Get pengaturan sesi aktif timbangan Formulasi Pasuruan",
     *     @OA\Parameter(
     *         name="token",
     *         in="query",
     *         required=true,
     *         description="Token unik perangkat timbangan",
     *         @OA\Schema(type="string", example="FORM-PASURUAN-001")
     *     ),
    ...
    public function getSettings(Request $request)
    {
        ...
        $operators = User::where('role', 'operator')
            ...
            ->where('tipe', 'formulasi_pasuruan') // Adjust if needed
            ->get();
        ...
        foreach ($operators as $op) {
            $sess = cache()->get("session_formulasi_{$op->id}");
            if ($sess) {
                $activeOperator = $op;
                $session = $sess;
                break;
    ...
    /**
     * @OA\Post(
     *     path="/api/v1/formulasi/weight",
     *     tags={"Formulasi Pasuruan"},
     *     summary="Kirim data berat timbangan Formulasi Pasuruan",
     *     @OA\RequestBody(
     *         required=true,
     *         @OA\MediaType(
     ...
     *                 @OA\Property(property="token", type="string", example="FORM-PASURUAN-001"),
     *                 @OA\Property(property="weight", type="number", format="float", example=10.25)
     *             )
     *         )
     *     ),
     *     @OA\Response(response=200, description="Sukses", @OA\JsonContent(ref="#/components/schemas/WeightSuccessResponse")),
    ...
    public function receiveWeight(Request $request)
    {
        ...
        try {
            broadcast(new WeightReceived('formulasi_pasuruan', [
                'weight' => $weight,
                'operator' => $operator->name,
    ...
    /**
     * @OA\Post(
     *     path="/api/v1/formulasi/ping",
     *     tags={"Formulasi Pasuruan"},
     *     summary="Ping perangkat timbangan Formulasi Pasuruan",
     *     @OA\RequestBody(
     *         required=true,
     *         @OA\MediaType(
     *             mediaType="application/x-www-form-urlencoded",
     ...
     *     path="/api/v1/formulasi/ping",
     *     tags={"Formulasi Pasuruan"},
     *     summary="Ping perangkat timbangan Formulasi Pasuruan (GET)",
     *     @OA\Parameter(name="token", in="query", required=true, @OA\Schema(type="string", example="FORM-PASURUAN-001")),
     *     @OA\Response(response=200, description="Pong", @OA\JsonContent(ref="#/components/schemas/PingResponse")),
     *     @OA\Response(response=401, description="Unauthorized", @OA\JsonContent(ref="#/components/schemas/ErrorResponse"))
     * )
     */
    public function ping(Request $request)
    {
        ...
        if ($device) {
            ...
            $operators = User::where('role', 'operator')->where('session_locked', false)->where('tipe', 'formulasi_pasuruan')->get();
            $session = null;
            $activeOperator = null;
