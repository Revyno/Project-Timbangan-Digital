...
class FgSurabayaIotController extends Controller
{
    /**
     * @OA\Get(
     *     path="/api/v1/fg-surabaya/settings",
     *     tags={"FG Surabaya"},
     *     summary="Get pengaturan sesi aktif timbangan FG Surabaya",
     *     @OA\Parameter(name="token", in="query", required=true, @OA\Schema(type="string", example="DEV-TOKEN-FG-SBY-001")),
    ...
    public function getSettings(Request $request)
    {
        ...
        foreach ($operators as $op) {
            $sess = cache()->get("session_fg_sby_{$op->id}");
            if ($sess) {
                $activeOperator = $op;
                $session = $sess;
                break;
    ...
    /**
     * @OA\Post(
     *     path="/api/v1/fg-surabaya/weight",
     *     tags={"FG Surabaya"},
     *     summary="Kirim data berat timbangan FG Surabaya",
     *     @OA\RequestBody(
     *         required=true,
     *         @OA\MediaType(
     ...
     *                 @OA\Property(property="token", type="string", example="DEV-TOKEN-FG-SBY-001"),
    ...
    /**
     * @OA\Post(
     *     path="/api/v1/fg-surabaya/ping",
     *     tags={"FG Surabaya"},
     *     summary="Ping perangkat timbangan FG Surabaya",
     ...
     *     path="/api/v1/fg-surabaya/ping",
     *     tags={"FG Surabaya"},
     *     summary="Ping perangkat timbangan FG Surabaya (GET)",
     *     @OA\Parameter(name="token", in="query", required=true, @OA\Schema(type="string", example="DEV-TOKEN-FG-SBY-001")),
     *     @OA\Response(response=200, description="Pong", @OA\JsonContent(ref="#/components/schemas/PingResponse")),
     *     @OA\Response(response=401, description="Token tidak valid", @OA\JsonContent(ref="#/components/schemas/ErrorResponse"))
     * )
