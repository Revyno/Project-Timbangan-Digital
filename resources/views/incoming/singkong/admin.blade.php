...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Data penimbangan incoming singkong (Read Only)</p>
        </div>
    ...
    <div class="p-6 bg-white border border-gray-200 rounded-2xl shadow-sm">
        <form method="GET" action="{{ route('incoming.singkong.dashboard') }}" class="space-y-4">
            <div class="grid grid-cols-1 gap-4">
                ...
                <div>
                    ...
                    <select name="supplier" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block w-full p-3 shadow-sm transition-all">
                        <option value="">Semua</option>
                        @foreach(\App\Models\IncomingSingkong::supplierOptions() as $s)
                            <option value="{{ $s }}" {{ request('supplier') == $s ? 'selected' : '' }}>{{ $s }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <a href="{{ route('incoming.singkong.export', request()->all()) }}" 
            ...
            <div class="flex">
                <a href="{{ route('incoming.singkong.dashboard') }}" 
