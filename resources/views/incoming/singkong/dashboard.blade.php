...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div>
            <h2 class="text-2xl font-black text-gray-800">Incoming Singkong Pasuruan</h2>
            <p class="text-sm text-gray-500 mt-1">Dashboard operator timbangan incoming singkong</p>
        </div>
    ...
    <div class="bg-white border border-gray-200 rounded-2xl shadow-md p-6">
            ...
            <div class="p-5 bg-emerald-500/10 border border-green-200 rounded-2xl">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <h4 class="text-2xl font-black text-gray-800 mt-1">{{ $activeSession['jenis_singkong'] }}</h4>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
                            ...
                            <div><span class="text-xs text-gray-500">Asal:</span><br><span class="text-xs font-bold text-gray-800">{{ $activeSession['asal'] }}</span></div>
                ...
                <div class="flex flex-col md:flex-row gap-3">
                    <!-- Tombol Ganti Produk -->
                    <form action="{{ route('incoming.singkong.next') }}" method="POST" class="w-full md:w-auto">
                        @csrf
                    ...
                    <form action="{{ route('incoming.singkong.stop') }}" method="POST" class="w-full md:w-auto">
                        @csrf
        ...
        @else
            <form action="{{ route('incoming.singkong.start') }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                @csrf
                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase mb-2 ml-1">Kode Produksi (LOT)</label>
<input type="text" name="kode_produksi" required placeholder="Contoh: TTS-LSI 00643"  class="w-full bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 p-3 placeholder-gray-400">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase mb-2 ml-1">No Surat</label>
<input type="text" name="no_surat" required placeholder="Contoh: TTS-LSI 00643"  class="w-full bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 p-3 placeholder-gray-400">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase mb-2 ml-1">Nama Supplier</label>
<select name="nama_supplier" required  class="w-full bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 p-3">
                        <option value="">-- Pilih Supplier --</option>
                        @foreach(\App\Models\IncomingSingkong::supplierOptions() as $s)
...
<select name="asal" required  class="w-full bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 p-3">
                        <option value="">-- Pilih Asal --</option>
                        @foreach(\App\Models\IncomingSingkong::asalOptions() as $a)
                            <option value="{{ $a }}">{{ $a }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase mb-2 ml-1">Nama Sopir</label>
<input type="text" name="nama_sopir" required placeholder="Nama sopir"  class="w-full bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 p-3 placeholder-gray-400">
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase mb-2 ml-1">Nomor Plat</label>
<input type="text" name="nomor_plat" required placeholder="Contoh: L 8150 CO"  class="w-full bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 p-3 placeholder-gray-400">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase mb-2 ml-1">Jenis Singkong</label>
<select name="jenis_singkong" required  class="w-full bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 p-3">
                        <option value="">-- Pilih Jenis --</option>
                        @foreach(\App\Models\IncomingSingkong::jenisOptions() as $j)
            ...
            <table class="w-full text-sm text-left text-gray-600">
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b border-gray-200 hover transition-colors">
                        ...
                        <td class="px-4 py-3">{{ $h->asal }}</td>
                        <td class="px-4 py-3">{{ $h->nama_sopir }}</td>
                        <td class="px-4 py-3">{{ $h->nomor_plat }}</td>
                        <td class="px-4 py-3">{{ $h->jenis_singkong }}</td>
                        <td class="px-4 py-3 font-bold text-gray-800">{{ number_format($h->berat, 0) }} kg</td>
                    </tr>
                    @endforeach
                    @if($history->isEmpty())
