...
<div class="space-y-6">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div>
            <h2 class="text-2xl font-black text-gray-800">Carton Sealer (FG-Sby)</h2>
            <p class="text-sm text-gray-500 mt-1">Dashboard operator CS FG Surabaya</p>
        </div>
    ...
    <div class="relative z-10 bg-white border border-gray-200 rounded-2xl shadow-md p-6">
        ...
        @if($activeSession)
            <div class="p-5 bg-emerald-500/10 border border-green-200 rounded-2xl">
                ...
                <div class="flex flex-col md:flex-row gap-3">
                    <!-- Tombol Ganti Produk -->
                    <form action="{{ route('cs-fg-sby.next') }}" method="POST" class="w-full md:w-auto">
                        @csrf
                    ...
                    <form action="{{ route('cs-fg-sby.stop') }}" method="POST" class="w-full md:w-auto">
                        @csrf
        ...
        @else
            <form action="{{ route('cs-fg-sby.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                @csrf
                <div>
                    <label class="block text-xs font-bold text-gray-700">Pilih Produk</label>
                        <select name="produk_id" id="produk_cs_fg_select" required class="w-full bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 p-3">
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        if (document.getElementById('produk_cs_fg_select')) {
            new TomSelect('#produk_cs_fg_select', { create: false, sortField: { field: "text", direction: "asc" }, placeholder: "-- Cari atau Pilih Produk --" });
        }
    });
    @if(session('success'))
