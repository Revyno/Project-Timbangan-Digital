...
<div class="space-y-6">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div>
            <h2 class="text-2xl font-black text-gray-800">Formulasi Surabaya</h2>
            <p class="text-sm text-gray-500 mt-1">Dashboard operator timbangan formulasi Surabaya</p>
        </div>
    ...
    <div class="relative z-10 bg-white border border-gray-200 rounded-2xl shadow-md p-6">
        ...
        @if($activeSession)
            <div class="p-5 bg-emerald-500/10 border border-green-200 rounded-2xl">
                ...
                <div class="flex flex-col md:flex-row gap-3">
                    <!-- Tombol Ganti Produk -->
                    <form action="{{ route('fg-surabaya.next') }}" method="POST" class="w-full md:w-auto">
                        @csrf
                    ...
                    <form action="{{ route('fg-surabaya.stop') }}" method="POST" class="w-full md:w-auto">
                        @csrf
        ...
        @else
            <form action="{{ route('fg-surabaya.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                @csrf
                <div>
                    <label class="block text-xs font-bold text-gray-700">produk_sby_select</label>
                        <select name="produk_id" id="produk_sby_select" required class="w-full bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 p-3">
...
<input type="date" name="tanggal_expired" required value="{{ $lastSession['tanggal_expired'] ?? '' }}" class="w-full bg-gray-50 border border-gray-300 text-gray-800 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 p-3">
                ...
                <div class="md:col-span-3">
                    <button type="submit" class="w-full md:w-auto px-10 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-all shadow-lg shadow-blue-500/20 flex items-center justify-center gap-2">
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        if (document.getElementById('produk_sby_select')) {
            new TomSelect('#produk_sby_select', { create: false, sortField: { field: "text", direction: "asc" }, placeholder: "-- Cari atau Pilih Produk --" });
        }
    });
    @if(session('success'))
